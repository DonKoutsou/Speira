...
class SP_DoorOpenerEntityClass : GenericEntityClass
{
};

class SP_DoorOpenerEntity : GenericEntity
{
	[Attribute(defvalue: "10", uiwidget: UIWidgets.Slider, desc: "Radius in which to open door.", "0 4000 1")]
	...
	override void EOnInit(IEntity owner)
	{
		...
		foreach (DoorStruct door : m_aQueriedDoors)
			door.component.SetControlValue(Math.RandomFloat(0.1, m_fControlValue));
	...
	//------------------------------------------------------------------------------------------------
	void SP_DoorOpenerEntity(IEntitySource src, IEntity parent)
	{
