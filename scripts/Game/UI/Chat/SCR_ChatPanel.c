...
class SCR_ChatPanel : ScriptedWidgetComponent
{
	...
	void SendContext(string contmessage)
	{
		SCR_ChatPanelManager mgr = SCR_ChatPanelManager.GetInstance();
		...
		;
		if (m_Widgets.m_MessageEditBox)
			contmessage = m_Widgets.m_MessageEditBox.GetText();
		else
			return;

		// Check if we want to send some command
		string cmd = this.GetCommand(contmessage);
		...
		{
			// Notify the chat panel mgr, pass the message with removed command
			contmessage = contmessage.Substring(cmd.Length() + 1, contmessage.Length() - cmd.Length() - 1);
			contmessage.TrimInPlace();	
			
			mgr.Internal_OnChatCommand(this, cmd, contmessage);
		...
		{	
			if (PrivateMessageChannel.Cast(m_ActiveChannel))
			{
				...
				{
					// Remove player name from the message
					contmessage = contmessage.Substring(nameLength + 1, contmessage.Length() - nameLength - 1);
					
					chatComponent.SendPrivateMessage(contmessage, playerId);
				}
			}
			else
			{
				int channelId = GetChannelId(m_ActiveChannel);
				chatComponent.SendMessage(contmessage, channelId);
			}
		}
	}
